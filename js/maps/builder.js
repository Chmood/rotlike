/******************************************************************************
 * BUILDER
 * Builds world map levels and connects regions with stairs
 ******************************************************************************/
...
Game.Builder.prototype._generateLevel = function(generatorType) {
  generatorType = generatorType || 'cellular';
  ...
  // Create a generator
  let _generator;
  if (generatorType === 'cellular') {
    // Setup the cave generator
    _generator = new ROT.Map.Cellular(this._width, this._height);
    _generator.randomize(0.5);
    ...
    _generator.create(function(x,y,v) {
      if (v === 1) {
  ...
  } else if (generatorType === 'digger') {
    _generator = new ROT.Map.Digger(
      this._width,
      this._height,
      {
        roomWidth: [4, 8], // [min, max] room size
        roomHeight: [4, 8], // [min, max] room size
        corridorLength: [4, 8], // [min, max] corridor length
        // algorithm stops after this fraction of map area has been dug out; default = 0.2
        dugPercentage: 0.5,
